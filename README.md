...
3) 카카오 장소검색                 →  1·2 가 모두 없을 때만 (최후 수단)
```
...
정적 데이터도 없고 응급의료포털도 죽었을 때만 쓴다. **카카오에는 영업시간이 없어
'지금 문 연 곳'을 가릴 수 없다.** 대신 위치·전화번호는 나오므로 전화로 확인할 수 있다.
아무것도 못 보여주는 것보다 낫다는 판단이다.

화면에 이 상태임을 분명히 알린다.

- 목록 위 경고: "영업시간을 확인할 수 없습니다 … 방문 전 전화로 확인해 주세요"
- 요약줄에 `· 카카오맵 기준`
- 카드 배지를 `영업 종료` 가 아니라 `영업시간 확인 불가` 로 (오해 방지)

한 번에 최대 45곳(15개 × 3페이지)까지만 받을 수 있는 API 제약이 있다. 거리순이라
가까운 곳부터 채워진다.
